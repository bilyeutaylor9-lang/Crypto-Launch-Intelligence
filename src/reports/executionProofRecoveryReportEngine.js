...
function projectIdentity(project = {}, rank = null) {
  ...
  return {
    ...
    failures: array(recovery.executionRecoveryFailures || route.executionRecoveryFailures).slice(0, 8),
    nextSingleProofToPromote:
      recovery.sellQuoteVerified !== true
        ? "fresh sell quote"
        : recovery.buyQuoteVerified !== true
          ? "fresh buy quote"
          : route.regionStatus === "UNKNOWN"
            ? "region/accessibility confirmation"
            : null,
    optionalSourceGaps: array(recovery.optionalSourceGaps).slice(0, 5),
    newlyPromotedToExecutionReview: recovery.newlyPromotedToExecutionReview === true,
    executionReady: project.executionReady === true || project.liveExecutionReady === true,
  };
}
...
export function summarizeExecutionProofRecovery(projects = [], meta = {}) {
  ...
  const executionReady = recoveredProjects.filter((project) => project.executionReady === true || project.liveExecutionReady === true);
