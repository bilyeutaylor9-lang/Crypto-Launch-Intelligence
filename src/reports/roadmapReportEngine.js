...
export const PRODUCT_ROADMAP = [
  {
    ...
    objective:
      "Detect why-now events before the normal scoring stack fully reacts.",
    deliverables: [
      "Catalyst event feed",
    ...
    commands: ["npm run scan", "npm run alerts", "npm run dossier-swarm"],
  },
  {
    month: 2,
    ...
    objective:
      "Turn the public dashboard into a live intelligence surface that people can inspect quickly.",
    deliverables: [
      "Catalyst feed",
      "Top dossier cards",
      "AI Council panel",
    ...
    commands: ["npm run demo", "npm run publish:dashboard"],
  },
  {
    month: 3,
    ...
    objective:
      "Measure which engines, narratives, and regimes actually produced follow-through.",
    deliverables: [
      "Backtest by engine",
    ...
    commands: ["npm run backtest", "npm run outcome-judge"],
  },
  {
    month: 4,
    ...
    objective:
      "Make every agent accountable for its calls and dynamically adjust trust.",
    deliverables: [
    ...
    commands: ["npm run agent-performance", "npm run agent-memory"],
  },
  {
    month: 5,
    ...
    objective:
      "Research projects from official and public sources with citations and freshness scoring.",
    deliverables: [
    ...
    commands: ["npm run research:internet", "npm run research:memory"],
  },
  {
    month: 6,
    ...
    objective:
      "Move from best-token ranking to portfolio-aware research decisions.",
    deliverables: [
      "Sector exposure",
      "Chain exposure",
      "Correlation risk",
    ...
    commands: ["npm run watchlist", "npm run dossier-swarm"],
  },
  {
    month: 7,
    ...
    objective:
      "Make the GitHub repo feel polished enough that visitors want to star, fork, and run it.",
    deliverables: [
    ...
    commands: ["npm run demo", "npm run engine:audit"],
  },
  {
    month: 8,
    ...
    objective:
      "Tie discovery, research, scoring, alerts, outcomes, and learning into one recurring research desk.",
    deliverables: [
      "Scheduled scans",
    ...
    commands: ["npm run watchtower:daemon", "npm run brief", "npm run publish:dashboard"],
  },
];
...
export function buildRoadmapReport() {
  return {
    ...
    vision:
      "A self-learning AI research desk: discover, research, score, debate, simulate, dossier, alert, track outcome, and learn.",
    horizonMonths: PRODUCT_ROADMAP.length,
    currentPhase: PRODUCT_ROADMAP[0],
...
export function writeRoadmapReport() {
  const reportsDir = path.resolve("reports");
  fs.mkdirSync(reportsDir, { recursive: true });

  const report = buildRoadmapReport();
  const filePath = path.join(reportsDir, "roadmap.json");
