...
export function runMarketDiscoveryCycle(options={}){
  ...
  appendJsonlDurable("data/market-discovery-predictions.jsonl",report.candidates.filter(row=>strictIdentity(row)&&Number(row.priceUsd??row.marketData?.priceUsd)>0).slice(0,100).map((row,index)=>({schemaVersion:1,generatedAt:now,decisionAt:now,rank:index+1,identityKey:row.identityKey,symbol:row.symbol||null,chain:row.chain||null,tokenAddress:row.tokenAddress||row.contractAddress||null,poolAddress:row.poolAddress||row.pairAddress||null,priceUsd:row.priceUsd??row.marketData?.priceUsd??null,alphaOSScore:row.governor?.alphaOSScore??null,captureableExpectedValuePct:row.executionAwareEV?.captureableExpectedValuePct??null,dependencyHealthPct:row.opportunityDependencies?.dependencyHealthPct??null,adversarialRiskScore:row.adversarialMarketIntelligence?.riskScore??null,forecastFitPct:report.predictiveDiscovery.matches.find(m=>m.identityKey===row.identityKey)?.fitScorePct??null,marketShockState:report.shockEarlyWarning.state,worldGraphId:report.worldModel.graphId,shadowOnly:true,productionInfluence:false})));
  return report;
}
