...
export function runAutonomousAlphaOSCycle(options = {}) {
  ...
  const inputFreshness=evaluateProspectiveSourceFreshness(candidateLoad.sourceObservedAt,now,{maximumSourceAgeMinutes:Number(options.maximumSourceAgeMinutes||90)});
  const marketObservationAudit={
    state:"RESEARCH_TRANSFORM_DOES_NOT_WRITE_MARKET_TRUTH",
    attempted:projects.length,
    saved:0,
    sourceObservedAt:candidateLoad.sourceObservedAt,
    policy:"Only acquisition/probe cycles append exact market observations; Alpha OS cannot relabel reused candidate data as a new observation.",
  };
  ...
  report.inputFreshness={...inputFreshness,prospectivePredictionEligible:inputFreshness.eligible};
