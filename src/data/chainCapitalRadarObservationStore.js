...
function compactObservation(observation = {}) {
  return {
    ...
    coveredThroughBlock: observation.coveredThroughBlock ?? observation.blockNumber ?? null,
    lookbackMinutes: observation.lookbackMinutes ?? null,
    lookbackBlocks: observation.lookbackBlocks ?? null,
    coveredMinutesEstimate: observation.coveredMinutesEstimate ?? null,
    continuityStatus: observation.continuityStatus || null,
    latestCoveredBlock: observation.latestCoveredBlock ?? null,
    continuityGapBlocks: observation.continuityGapBlocks ?? null,
    coverageComplete: observation.coverageComplete === true,
    qualifyingTransferCount: observation.qualifyingTransferCount ?? 0,
    fundedRecipientCount: observation.fundedRecipientCount ?? 0,
    carriedWalletCount: observation.carriedWalletCount ?? 0,
    candidateWalletCount: observation.candidateWalletCount ?? 0,
    contractRecipientCount: observation.contractRecipientCount ?? 0,
    zeroBalanceWalletCount: observation.zeroBalanceWalletCount ?? 0,
    gasNotReadyWalletCount: observation.gasNotReadyWalletCount ?? 0,
    noActiveApprovalWalletCount: observation.noActiveApprovalWalletCount ?? 0,
    ...
    wallets: (observation.wallets || []).slice(0, 120).map((row) => ({
      ...
      carriedForward: Boolean(row.carriedForward),
