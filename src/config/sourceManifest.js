export const SOURCE_STATUS = {
  PLANNED: "PLANNED",
  IMPLEMENTED: "IMPLEMENTED",
  ENABLED: "ENABLED",
  DEGRADED: "DEGRADED",
};

export const EVIDENCE_FAMILIES = {
  market: "market-aggregator",
  chain: "chain-event",
  project: "project-controlled",
  news: "independent-news",
  developer: "developer-activity",
  wallet: "wallet-activity",
  security: "security-analysis",
  funding: "funding-evidence",
  community: "community-activity",
  exchange: "exchange-evidence",
  research: "research-seed",
};

export const SOURCE_MANIFEST = [
  {
    id: "dexscreener",
    label: "DexScreener",
    category: "market",
    evidenceFamily: EVIDENCE_FAMILIES.market,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["ethereum", "base", "arbitrum", "polygon", "bsc", "solana"],
  },
  {
    id: "geckoterminal",
    label: "GeckoTerminal",
    category: "market",
    evidenceFamily: EVIDENCE_FAMILIES.market,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["ethereum", "base", "arbitrum", "polygon", "bsc", "solana"],
  },
  {
    id: "coingecko",
    label: "CoinGecko",
    category: "market",
    evidenceFamily: EVIDENCE_FAMILIES.market,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["multi-chain"],
  },
  {
    id: "birdeye",
    label: "Birdeye",
    category: "market",
    evidenceFamily: EVIDENCE_FAMILIES.market,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: true,
    chains: ["solana", "ethereum", "base", "polygon", "bsc"],
  },
  {
    id: "freeMarketData",
    label: "Free Market Providers",
    category: "market",
    evidenceFamily: EVIDENCE_FAMILIES.market,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["multi-chain"],
  },
  {
    id: "coinpaprika",
    label: "CoinPaprika",
    category: "market",
    evidenceFamily: EVIDENCE_FAMILIES.market,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["multi-chain"],
  },
  {
    id: "coinlore",
    label: "CoinLore Market Tickers",
    category: "market",
    evidenceFamily: EVIDENCE_FAMILIES.market,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["multi-chain"],
  },
  {
    id: "coinlore-assets",
    label: "CoinLore Asset Directory",
    category: "market",
    evidenceFamily: EVIDENCE_FAMILIES.market,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["multi-chain"],
  },
  {
    id: "coinlore-movers",
    label: "CoinLore Movers",
    category: "market",
    evidenceFamily: EVIDENCE_FAMILIES.market,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["multi-chain"],
  },
  {
    id: "defillama",
    label: "DeFiLlama Protocol TVL",
    category: "market",
    evidenceFamily: EVIDENCE_FAMILIES.project,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["multi-chain"],
  },
  {
    id: "defillama-chain",
    label: "DeFiLlama Chain TVL",
    category: "market",
    evidenceFamily: EVIDENCE_FAMILIES.project,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: false,
    requiresKey: false,
    chains: ["multi-chain"],
  },
  {
    id: "defillama-yields",
    label: "DeFiLlama Yield Pools",
    category: "market",
    evidenceFamily: EVIDENCE_FAMILIES.project,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["multi-chain"],
  },
  {
    id: "defillama-stablecoins",
    label: "DeFiLlama Stablecoins",
    category: "market",
    evidenceFamily: EVIDENCE_FAMILIES.market,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["multi-chain"],
  },
  {
    id: "defillama-exact-price",
    label: "DeFiLlama Exact Contract Price",
    category: "market",
    evidenceFamily: EVIDENCE_FAMILIES.market,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: false,
    enrichmentProvider: true,
    requiresKey: false,
    accessMode: "KEYLESS_PUBLIC",
    chains: ["ethereum", "base", "bsc", "arbitrum", "polygon", "optimism", "avalanche", "solana", "multi-chain"],
  },
  {
    id: "point-in-time-market-context",
    label: "Point-in-Time Market Context",
    category: "market",
    evidenceFamily: EVIDENCE_FAMILIES.market,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: false,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["multi-chain"],
  },
  {
    id: "hyperliquid-public-context",
    label: "Hyperliquid Public Derivatives Context",
    category: "exchange",
    evidenceFamily: EVIDENCE_FAMILIES.exchange,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: false,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["exchange-market"],
  },
  {
    id: "lifi",
    label: "LI.FI Read-Only Quote API",
    category: "execution",
    evidenceFamily: EVIDENCE_FAMILIES.exchange,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: false,
    enrichmentProvider: true,
    requiresKey: false,
    accessMode: "KEYLESS_PUBLIC",
    chains: ["ethereum", "base", "bsc", "arbitrum", "polygon", "optimism", "avalanche", "multi-chain-evm"],
  },
  {
    id: "jupiter",
    label: "Jupiter Quote API",
    category: "execution",
    evidenceFamily: EVIDENCE_FAMILIES.exchange,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: false,
    enrichmentProvider: true,
    requiresKey: true,
    accessMode: "API_KEY_REQUIRED",
    chains: ["solana"],
  },
  {
    id: "zerox",
    label: "0x Swap API",
    category: "execution",
    evidenceFamily: EVIDENCE_FAMILIES.exchange,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: false,
    enrichmentProvider: true,
    requiresKey: true,
    accessMode: "API_KEY_REQUIRED",
    chains: ["multi-chain-evm"],
  },
  {
    id: "binance-us",
    label: "Binance.US",
    category: "exchange",
    evidenceFamily: EVIDENCE_FAMILIES.exchange,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["exchange-market"],
  },
  {
    id: "binance",
    label: "Binance",
    category: "exchange",
    evidenceFamily: EVIDENCE_FAMILIES.exchange,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["exchange-market"],
  },
  {
    id: "coinbase",
    label: "Coinbase Exchange",
    category: "exchange",
    evidenceFamily: EVIDENCE_FAMILIES.exchange,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["exchange-market"],
  },
  {
    id: "kraken",
    label: "Kraken",
    category: "exchange",
    evidenceFamily: EVIDENCE_FAMILIES.exchange,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["exchange-market"],
  },
  {
    id: "kucoin",
    label: "KuCoin",
    category: "exchange",
    evidenceFamily: EVIDENCE_FAMILIES.exchange,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["exchange-market"],
  },
  {
    id: "okx",
    label: "OKX",
    category: "exchange",
    evidenceFamily: EVIDENCE_FAMILIES.exchange,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["exchange-market"],
  },
  {
    id: "bybit",
    label: "Bybit",
    category: "exchange",
    evidenceFamily: EVIDENCE_FAMILIES.exchange,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["exchange-market"],
  },
  {
    id: "gate",
    label: "Gate.io",
    category: "exchange",
    evidenceFamily: EVIDENCE_FAMILIES.exchange,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["exchange-market"],
  },
  {
    id: "mexc",
    label: "MEXC",
    category: "exchange",
    evidenceFamily: EVIDENCE_FAMILIES.exchange,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["exchange-market"],
  },
  {
    id: "bitget",
    label: "Bitget",
    category: "exchange",
    evidenceFamily: EVIDENCE_FAMILIES.exchange,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["exchange-market"],
  },
  {
    id: "htx",
    label: "HTX",
    category: "exchange",
    evidenceFamily: EVIDENCE_FAMILIES.exchange,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["exchange-market"],
  },
  {
    id: "bitfinex",
    label: "Bitfinex",
    category: "exchange",
    evidenceFamily: EVIDENCE_FAMILIES.exchange,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["exchange-market"],
  },
  {
    id: "bitstamp",
    label: "Bitstamp",
    category: "exchange",
    evidenceFamily: EVIDENCE_FAMILIES.exchange,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["exchange-market"],
  },
  {
    id: "gemini",
    label: "Gemini",
    category: "exchange",
    evidenceFamily: EVIDENCE_FAMILIES.exchange,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["exchange-market"],
  },
  {
    id: "coincap",
    label: "CoinCap",
    category: "market",
    evidenceFamily: EVIDENCE_FAMILIES.market,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: true,
    chains: ["multi-chain"],
  },
  {
    id: "cryptocompare",
    label: "CryptoCompare",
    category: "market",
    evidenceFamily: EVIDENCE_FAMILIES.market,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: true,
    chains: ["multi-chain"],
  },
  {
    id: "expandedMarketData",
    label: "Expanded Free Market Providers",
    category: "market",
    evidenceFamily: EVIDENCE_FAMILIES.market,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["multi-chain"],
  },
  {
    id: "dexscreener-community-takeovers",
    label: "DexScreener Community Takeovers",
    category: "market",
    evidenceFamily: EVIDENCE_FAMILIES.community,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["ethereum", "base", "arbitrum", "polygon", "bsc", "solana"],
  },
  {
    id: "dexscreener-ads",
    label: "DexScreener Ads",
    category: "market",
    evidenceFamily: EVIDENCE_FAMILIES.community,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["ethereum", "base", "arbitrum", "polygon", "bsc", "solana"],
  },
  {
    id: "googleNewsDiscovery",
    label: "Google News Discovery",
    category: "news",
    evidenceFamily: EVIDENCE_FAMILIES.news,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["multi-chain"],
  },
  {
    id: "githubProjectDiscovery",
    label: "GitHub Project Discovery",
    category: "developer",
    evidenceFamily: EVIDENCE_FAMILIES.developer,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["prelaunch", "multi-chain"],
  },
  {
    id: "nativeDiscoveryMesh",
    label: "Native Discovery Mesh",
    category: "chain",
    evidenceFamily: EVIDENCE_FAMILIES.chain,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["base", "solana", "ethereum", "bsc", "polygon", "arbitrum", "avalanche"],
  },
  {
    id: "researchSeeds",
    label: "Fallback Research Seeds",
    category: "research",
    evidenceFamily: EVIDENCE_FAMILIES.research,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: false,
    requiresKey: false,
    chains: ["multi-chain"],
  },
  {
    id: "aiDiscoverySwarm",
    label: "AI Discovery Swarm",
    category: "research",
    evidenceFamily: EVIDENCE_FAMILIES.research,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["multi-chain"],
  },
  {
    id: "candidateRescue",
    label: "Candidate Rescue",
    category: "research",
    evidenceFamily: EVIDENCE_FAMILIES.research,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["multi-chain"],
  },
  {
    id: "evmFactoryEvents",
    label: "EVM Factory Events",
    category: "chain",
    evidenceFamily: EVIDENCE_FAMILIES.chain,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["ethereum", "base", "arbitrum", "polygon", "bsc"],
  },
  {
    id: "solanaProgramEvents",
    label: "Solana Program Events",
    category: "chain",
    evidenceFamily: EVIDENCE_FAMILIES.chain,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["solana"],
  },
  {
    id: "etherscan",
    label: "Etherscan V2 Contract Explorer",
    category: "security",
    evidenceFamily: EVIDENCE_FAMILIES.security,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: false,
    enrichmentProvider: true,
    requiresKey: true,
    chains: ["ethereum", "base", "bsc", "polygon", "arbitrum", "optimism", "avalanche"],
  },
  {
    id: "goplus",
    label: "GoPlus Security",
    category: "security",
    evidenceFamily: EVIDENCE_FAMILIES.security,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: false,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["ethereum", "base", "bsc", "polygon", "arbitrum", "optimism", "avalanche", "solana"],
  },
  {
    id: "sourcify-v2",
    label: "Sourcify Contract Verification",
    category: "security",
    evidenceFamily: EVIDENCE_FAMILIES.security,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: false,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["ethereum", "base", "bsc", "polygon", "arbitrum", "optimism", "avalanche"],
  },
  {
    id: "blockscout",
    label: "Blockscout Contract Metadata",
    category: "security",
    evidenceFamily: EVIDENCE_FAMILIES.security,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: false,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["ethereum", "base", "bsc", "polygon", "arbitrum", "optimism", "avalanche"],
  },
  {
    id: "rugcheck",
    label: "RugCheck",
    category: "security",
    evidenceFamily: EVIDENCE_FAMILIES.security,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: false,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["solana"],
  },
  {
    id: "launchpads",
    label: "Launchpads",
    category: "launch",
    evidenceFamily: EVIDENCE_FAMILIES.project,
    status: SOURCE_STATUS.IMPLEMENTED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["multi-chain"],
  },
  {
    id: "ecosystemGrants",
    label: "Ecosystem Grants",
    category: "funding",
    evidenceFamily: EVIDENCE_FAMILIES.funding,
    status: SOURCE_STATUS.PLANNED,
    candidateGenerator: true,
    enrichmentProvider: true,
    requiresKey: false,
    chains: ["multi-chain"],
  },
];

const SOURCE_ALIASES = new Map([
  ["dexscreener-search", "dexscreener"],
  ["dexscreener-profiles", "dexscreener"],
  ["dexscreener-boosts", "dexscreener"],
  ["dexscreener-latest", "dexscreener"],
  ["dexscreener-boosted", "dexscreener"],
  ["dexscreener-trending", "dexscreener"],
  ["dexscreener-community", "dexscreener-community-takeovers"],
  ["dexscreener-community-takeover", "dexscreener-community-takeovers"],
  ["dexscreener-community-takeovers", "dexscreener-community-takeovers"],
  ["dexscreener-ad", "dexscreener-ads"],
  ["dexscreener-ads", "dexscreener-ads"],
  ["coin-gecko", "coingecko"],
  ["coincap", "coincap"],
  ["coin-cap", "coincap"],
  ["cryptocompare", "cryptocompare"],
  ["crypto-compare", "cryptocompare"],
  ["coinlore", "coinlore"],
  ["coinpaprika-market", "coinpaprika"],
  ["coinlore-assets", "coinlore-assets"],
  ["coinlore-movers", "coinlore-movers"],
  ["defillama-protocol", "defillama"],
  ["defillama-chain-tvl", "defillama-chain"],
  ["defillama-yield", "defillama-yields"],
  ["defillama-yields", "defillama-yields"],
  ["defillama-stablecoin", "defillama-stablecoins"],
  ["defillama-stablecoins", "defillama-stablecoins"],
  ["defillama-exact", "defillama-exact-price"],
  ["defillama-exact-price", "defillama-exact-price"],
  ["defillama-price", "defillama-exact-price"],
  ["li.fi", "lifi"],
  ["li-fi", "lifi"],
  ["lifi-keyless", "lifi"],
  ["jupiter-quote", "jupiter"],
  ["0x", "zerox"],
  ["0x-swap-api", "zerox"],
  ["market-context", "point-in-time-market-context"],
  ["hyperliquid", "hyperliquid-public-context"],
  ["etherscan-v2", "etherscan"],
  ["etherscan api", "etherscan"],
  ["etherscan-api", "etherscan"],
  ["ethscan", "etherscan"],
  ["ethereum-explorer", "etherscan"],
  ["basescan", "etherscan"],
  ["bscscan", "etherscan"],
  ["arbiscan", "etherscan"],
  ["polygonscan", "etherscan"],
  ["optimistic-etherscan", "etherscan"],
  ["snowtrace", "etherscan"],
  ["gecko-terminal", "geckoterminal"],
  ["google-news", "googleNewsDiscovery"],
  ["github-project-discovery", "githubProjectDiscovery"],
  ["native-discovery-mesh", "nativeDiscoveryMesh"],
  ["native-mesh", "nativeDiscoveryMesh"],
  ["native-pool-events", "nativeDiscoveryMesh"],
  ["research-seed", "researchSeeds"],
  ["research-seed-supplement", "researchSeeds"],
  ["ai-discovery-swarm", "aiDiscoverySwarm"],
  ["candidate-rescue", "candidateRescue"],
  ["free-market", "freeMarketData"],
  ["expanded-market", "expandedMarketData"],
  ["binanceus", "binance-us"],
  ["binance.us", "binance-us"],
  ["coinbase-exchange", "coinbase"],
  ["okx-exchange", "okx"],
  ["gateio", "gate"],
  ["gate.io", "gate"],
  ["mexc-global", "mexc"],
  ["bitget-spot", "bitget"],
  ["huobi", "htx"],
]);

const SOURCE_TRUTH_KINDS = {
  dexscreener: "dex",
  geckoterminal: "dex",
  nativeDiscoveryMesh: "dex",
  "dexscreener-community-takeovers": "dex",
  "dexscreener-ads": "dex",
  coingecko: "aggregate-market",
  "coingecko-trending": "aggregate-market",
  coinpaprika: "aggregate-market",
  coinlore: "aggregate-market",
  "coinlore-assets": "aggregate-market",
  "coinlore-movers": "aggregate-market",
  coincap: "aggregate-market",
  cryptocompare: "aggregate-market",
  "defillama-stablecoins": "aggregate-market",
  "defillama-exact-price": "aggregate-market",
  "point-in-time-market-context": "aggregate-market",
  "hyperliquid-public-context": "cex",
  lifi: "execution-quote",
  jupiter: "execution-quote",
  zerox: "execution-quote",
  binance: "cex",
  "binance-us": "cex",
  coinbase: "cex",
  kraken: "cex",
  kucoin: "cex",
  okx: "cex",
  bybit: "cex",
  gate: "cex",
  mexc: "cex",
  bitget: "cex",
  htx: "cex",
  bitfinex: "cex",
  bitstamp: "cex",
  gemini: "cex",
  defillama: "protocol-tvl",
  "defillama-chain": "protocol-tvl",
  "defillama-yields": "protocol-tvl",
  googleNewsDiscovery: "unresolved-news",
  githubProjectDiscovery: "unresolved-repository",
  researchSeeds: "research-seed",
  aiDiscoverySwarm: "research-seed",
  candidateRescue: "research-seed",
};

export function canonicalSourceId(source = "") {
  const raw = String(source || "").trim();
  if (!raw) return "unknown";
  const normalized = raw.replace(/[_\s]+/g, "-");
  const lower = normalized.toLowerCase();
  const exact = SOURCE_MANIFEST.find((item) => item.id === raw || item.id.toLowerCase() === lower);
  if (exact) return exact.id;
  return SOURCE_ALIASES.get(raw) || SOURCE_ALIASES.get(lower) || lower;
}

export function getSourceManifest() {
  return SOURCE_MANIFEST.map((source) => ({
    lastSuccessAt: null,
    uniqueCandidates24h: 0,
    overlapRate: 0,
    errorRate: 0,
    cursor: null,
    ...source,
  }));
}

export function getSourceById(source = "") {
  const id = canonicalSourceId(source);
  return getSourceManifest().find((item) => item.id === id) || {
    id,
    label: id,
    category: "unknown",
    evidenceFamily: "unknown",
    status: SOURCE_STATUS.PLANNED,
    candidateGenerator: false,
    enrichmentProvider: false,
    requiresKey: false,
    chains: [],
    lastSuccessAt: null,
    uniqueCandidates24h: 0,
    overlapRate: 0,
    errorRate: 0,
    cursor: null,
  };
}

export function evidenceFamilyForSource(source = "") {
  return getSourceById(source).evidenceFamily || "unknown";
}

export function truthKindForSource(source = "") {
  const id = canonicalSourceId(source);
  return SOURCE_TRUTH_KINDS[id] || "unknown";
}

export function groupSourcesByCategory() {
  return getSourceManifest().reduce((acc, source) => {
    acc[source.category] = acc[source.category] || [];
    acc[source.category].push(source.id);
    return acc;
  }, {});
}

export function summarizeSourceManifest() {
  const sources = getSourceManifest();
  const implemented = sources.filter((source) => source.status !== SOURCE_STATUS.PLANNED);
  const candidateGenerators = sources.filter((source) => source.candidateGenerator);
  const enrichmentProviders = sources.filter((source) => source.enrichmentProvider);

  return {
    totalSources: sources.length,
    implementedSources: implemented.length,
    plannedSources: sources.length - implemented.length,
    candidateGenerators: candidateGenerators.length,
    enrichmentProviders: enrichmentProviders.length,
    categories: Object.keys(groupSourcesByCategory()),
    sourcesByCategory: groupSourcesByCategory(),
    sources,
  };
}
