  ...
  for (const provider of ["coinbase", "kraken", "gate", "mexc"]) {
    try {
      ...
      const route = {
        ...
        executableDepthUsd: book.orderBookDepthUsd,
        verifiedTradeSizeUsd: Math.min(num(book.orderBookDepthUsd), 500),
        verifiedDepthSource: "public-order-book",
...
async function recoverProject(project = {}, recoveryMeta = {}, options = {}, signal = null) {
  ...
  const canonicalExecutionRoute = {
    ...
    quoteTokenAddress: bestRoute.quoteTokenAddress || project.canonicalExecutionRoute?.quoteTokenAddress || null,
    ...
    executableDepthUsd: bestRoute.executableDepthUsd ?? project.canonicalExecutionRoute?.executableDepthUsd ?? null,
    verifiedTradeSizeUsd: bestRoute.verifiedTradeSizeUsd ?? project.canonicalExecutionRoute?.verifiedTradeSizeUsd ?? null,
    verifiedDepthSource: bestRoute.verifiedDepthSource || project.canonicalExecutionRoute?.verifiedDepthSource || null,
    priceUsd: bestRoute.priceUsd ?? priceUsd(project) ?? null,
    priceImpactPct: bestRoute.priceImpactPct ?? project.canonicalExecutionRoute?.priceImpactPct ?? null,
    estimatedRoundTripSlippagePct: bestRoute.estimatedRoundTripSlippagePct ?? project.canonicalExecutionRoute?.estimatedRoundTripSlippagePct ?? null,
    slippagePct: bestRoute.slippagePct ?? project.canonicalExecutionRoute?.slippagePct ?? null,
    slippageIsHeuristic: bestRoute.slippageIsHeuristic === true,
    quoteTimestamp: bestRoute.quoteTimestamp || null,
    quoteAgeSeconds: bestRoute.quoteAgeSeconds ?? null,
    regionStatus: bestRoute.regionStatus || project.canonicalExecutionRoute?.regionStatus || null,
    executionRecoverySource: bestRoute.executionRecoverySource || project.canonicalExecutionRoute?.executionRecoverySource || null,
  ...
  const recoveredProject = {
    ...
    slippageIsHeuristic: false,
    executableDepthUsd: Math.max(num(project.executableDepthUsd), num(bestRoute.executableDepthUsd)),
    verifiedTradeSizeUsd: Math.max(num(project.verifiedTradeSizeUsd), num(bestRoute.verifiedTradeSizeUsd)),
    verifiedDepthSource: bestRoute.verifiedDepthSource || project.verifiedDepthSource,
