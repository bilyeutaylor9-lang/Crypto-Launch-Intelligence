...
-- Durable append-only system of record for point-in-time forward evidence.
create table if not exists public.forward_evidence_records (
  ...
  constraint forward_evidence_ledger_name_check check (
    ledger_name in ('production-market-observations', 'market-context-observations', 'prospective-edge-cohorts')
  )
);
